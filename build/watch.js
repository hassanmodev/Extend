...
const getSettingsFile = fileName => {
    try {
        ...
        global.settingsFile = Object.assign(Object.assign({}, global.settingsFile), userRules);
        global.settings = Object.assign(Object.assign({}, global.settings), userRules.settings || {});
        return userRules;
    }
...
var getUserRules = (fileName) => {
    ...
    for (let i = 0; i < maxAttempets; i++) {
        if (!Object.keys(settingsFile).length) {
            settingsFile = getSettingsFile(fileName);
        }
        else
            break;
    }
...
var writeToFile = (processed, fileName) => {
    if (!processed)
        return global.msg(`${fileName}: got nothing to write.`);
    processed = (0, js_beautify_1.default)(processed);
...
var localCompile = (fileName) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // again, vscode
    yield (new Promise(res => setTimeout(res, minTime)));
    var shoudCompile = name => {
        var nameList = name.split(".");
        if (path.basename(name) === rulesFileName) {
            return 'rules';
        }
        if (nameList.slice(-1).join() === extentions[0].join())
            return "xt";
        if (nameList.slice(-2).join() === extentions[1].join())
            return "xt.js";
        return false;
    };
    ...
    if (shouldCompile === "xt.js")
        writeName = writeName.replace(".xt", "");
...
const startWatching = () => {
    ...
    settings = Object.assign(Object.assign({}, defaultSettings), settingsFile.settings);
