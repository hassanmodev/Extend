...
const compileBlock = (sourceCode, userRules) => {
    try {
        let tokenized = (0, parse_2.parseCode)(sourceCode);
...
const processCode = (sourceCode, userRules, fileName = '') => {
    ...
    for (let i = 0; i < sourceCode.length; i++) {
        ...
        // nested calls
        if (foundOpenCode) {
            var res = processCode(sourceCode.slice(i + codeMarkers[0].length), userRules).text;
            if (res) {
        ...
        // code finished.. return
            ...
            outputText += letter.value || letter;
        }
    }
    if (accumulator)
        outputText += compileBlock(accumulator, userRules);
...
const getVariables = (rule, toknized, wordAfterArray = 0, index = 'unknown') => {
    // rmv codemarkers
    ...
    while (true) {
        ...
        for (var foundIndex = templateRealIndex + template_index_adjust; foundIndex < toknized.length; foundIndex++) {
            ...
            if (wordAfterArray)
                if (nextTempWord && wordAfterArray.value === foundWord.value)
                    breakWhile = true;
            ...
            if (isRightKeyword(foundWord, rule, templateIndex + 1)) {
                ...
                if (!(0, parse_1.unbalanced)(lastFoundVar)) {
                    if (!breakFor && !breakWhile) {
                        skipI = true;
                        break;
                    }
...
var variableMatchRule = (variables, foundVariables) => {
    for (var variable of variables) {
        ...
        if (!extractedValue || (Array.isArray(extractedValue) && !extractedValue.length)) {
            settings_1.default.showNotFound && console.log(variable.value || variable.name, 'not found in', foundVariables);
            return false;
        }
        if (!variable.rest)
            continue;
