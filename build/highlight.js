...
var start = (userRules, terminals) => {
    ...
    if (!settings)
        return false;
    if (!global.settings.vscodeHighlighting)
        return clearSettings(settings);
    let rules = userRules;
    settings["highlight.regexes"] = {};
    rules.forEach((rule) => {
        let block = createRegex(rule.parsed, terminals);
        settings["highlight.regexes"] = Object.assign(Object.assign({}, settings["highlight.regexes"]), block);
    });
    // default case
    settings["highlight.regexes"][`(${terminals[0]})${anyButWord(terminals[1])}(${terminals[1]})`] = [colors.terminals, colors.default, colors.terminals];
    writeSettings(settings);
};
exports.start = start;
